...
//********************************* *******//
// Passed SigLow object to CreateSubOrder
//
// 30 % : Selling at PRICE PAID + GAP; Stop Loss = MaxBreach
// 40 %: Selling at PRICE PAID + (2 * GAP); Stop Loss is at PRICE PAID
// 30 %  Selling at PRICE PAID + (3 * GAP); Stop Loss is at PRICE PAID + GAP (a.k.a OTO level)
//********************************
...
public class BuyOrder
{
    ...
    public BuyOrder(Robot _bot, SignificantLow _sigLowObject, DateTime _dateBought, double _pricePaid, double _gap, double _unitsBought)
    {
        try
        {
            ...
            State = "ACTIVE";
            ActiveSubOrder = 1;
            SubOrders = createSubOrders(_robot, _sigLowObject.Id, _pricePaid, _gap, _unitsBought, _sigLowObject.Price ,_sigLowObject);
    ...
    protected Dictionary<int, SubOrder> createSubOrders(Robot _bot, long _id, double _pricePaid, double _gap, double _unitsBought, double _sigLowPrice, SignificantLow _sigLowObject)
    {
        ...
        try
        {
            ...
            for (int i = 1; i <= _ratio.Length; i++)
            {
                ...
                if (i == 1)
                {
                    ...
                    stopLoss = _sigLowObject.MaxBreach;
                }
                else if (i == 2)
                {
